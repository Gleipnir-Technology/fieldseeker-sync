...
func (s *HistoryPolygonlocationSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if !s.Filter.IsUnset() {
			vals[10] = psql.Arg(s.Filter.MustGetNull())
		} else {
		...
		if !s.Locationnumber.IsUnset() {
			vals[30] = psql.Arg(s.Locationnumber.MustGetNull())
		} else {
			vals[30] = psql.Raw("DEFAULT")
		}

		if !s.Name.IsUnset() {
			vals[31] = psql.Arg(s.Name.MustGetNull())
		} else {
			vals[31] = psql.Raw("DEFAULT")
		}

		if !s.Nextactiondatescheduled.IsUnset() {
			vals[32] = psql.Arg(s.Nextactiondatescheduled.MustGetNull())
		} else {
			vals[32] = psql.Raw("DEFAULT")
		}

		if s.Objectid.IsValue() {
			vals[33] = psql.Arg(s.Objectid.MustGet())
		} else {
			vals[33] = psql.Raw("DEFAULT")
		}

		if !s.Priority.IsUnset() {
			vals[34] = psql.Arg(s.Priority.MustGetNull())
		} else {
		...
		if !s.ShapeArea.IsUnset() {
			vals[36] = psql.Arg(s.ShapeArea.MustGetNull())
		} else {
			vals[36] = psql.Raw("DEFAULT")
		}

		if !s.ShapeLength.IsUnset() {
			vals[37] = psql.Arg(s.ShapeLength.MustGetNull())
		} else {
			vals[37] = psql.Raw("DEFAULT")
		}

		if !s.Usetype.IsUnset() {
			vals[38] = psql.Arg(s.Usetype.MustGetNull())
		} else {
			vals[38] = psql.Raw("DEFAULT")
		}

		if !s.Waterorigin.IsUnset() {
			vals[39] = psql.Arg(s.Waterorigin.MustGetNull())
		} else {
