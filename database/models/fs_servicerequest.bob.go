...
func (s *FSServicerequestSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if !s.Duedate.IsUnset() {
			vals[22] = psql.Arg(s.Duedate.MustGetNull())
		} else {
			vals[22] = psql.Raw("DEFAULT")
		}

		if !s.Entrytech.IsUnset() {
			vals[23] = psql.Arg(s.Entrytech.MustGetNull())
		} else {
			vals[23] = psql.Raw("DEFAULT")
		}

		if !s.Estcompletedate.IsUnset() {
			vals[24] = psql.Arg(s.Estcompletedate.MustGetNull())
		} else {
			vals[24] = psql.Raw("DEFAULT")
		}

		if !s.Externalerror.IsUnset() {
			vals[25] = psql.Arg(s.Externalerror.MustGetNull())
		} else {
			vals[25] = psql.Raw("DEFAULT")
		}

		if !s.Externalid.IsUnset() {
			vals[26] = psql.Arg(s.Externalid.MustGetNull())
		} else {
		...
		if !s.Firstresponsedate.IsUnset() {
			vals[29] = psql.Arg(s.Firstresponsedate.MustGetNull())
		} else {
			vals[29] = psql.Raw("DEFAULT")
		}

		if !s.Globalid.IsUnset() {
			vals[30] = psql.Arg(s.Globalid.MustGetNull())
		} else {
			vals[30] = psql.Raw("DEFAULT")
		}

		if !s.Issuesreported.IsUnset() {
			vals[31] = psql.Arg(s.Issuesreported.MustGetNull())
		} else {
		...
		if !s.Nextaction.IsUnset() {
			vals[33] = psql.Arg(s.Nextaction.MustGetNull())
		} else {
			vals[33] = psql.Raw("DEFAULT")
		}

		if !s.Notificationtimestamp.IsUnset() {
			vals[34] = psql.Arg(s.Notificationtimestamp.MustGetNull())
		} else {
			vals[34] = psql.Raw("DEFAULT")
		}

		if !s.Notified.IsUnset() {
			vals[35] = psql.Arg(s.Notified.MustGetNull())
		} else {
			vals[35] = psql.Raw("DEFAULT")
		}

		if !s.Notifieddate.IsUnset() {
			vals[36] = psql.Arg(s.Notifieddate.MustGetNull())
		} else {
			vals[36] = psql.Raw("DEFAULT")
		}

		if s.Objectid.IsValue() {
			vals[37] = psql.Arg(s.Objectid.MustGet())
		} else {
			vals[37] = psql.Raw("DEFAULT")
		}

		if !s.Pointlocid.IsUnset() {
			vals[38] = psql.Arg(s.Pointlocid.MustGetNull())
		} else {
			vals[38] = psql.Raw("DEFAULT")
		}

		if !s.Priority.IsUnset() {
			vals[39] = psql.Arg(s.Priority.MustGetNull())
		} else {
			vals[39] = psql.Raw("DEFAULT")
		}

		if !s.Recdatetime.IsUnset() {
			vals[40] = psql.Arg(s.Recdatetime.MustGetNull())
		} else {
			vals[40] = psql.Raw("DEFAULT")
		}

		if !s.Recordstatus.IsUnset() {
			vals[41] = psql.Arg(s.Recordstatus.MustGetNull())
		} else {
			vals[41] = psql.Raw("DEFAULT")
		}

		if !s.Rejectedby.IsUnset() {
			vals[42] = psql.Arg(s.Rejectedby.MustGetNull())
		} else {
			vals[42] = psql.Raw("DEFAULT")
		}

		if !s.Rejecteddate.IsUnset() {
			vals[43] = psql.Arg(s.Rejecteddate.MustGetNull())
		} else {
			vals[43] = psql.Raw("DEFAULT")
		}

		if !s.Rejectedreason.IsUnset() {
			vals[44] = psql.Arg(s.Rejectedreason.MustGetNull())
		} else {
			vals[44] = psql.Raw("DEFAULT")
		}

		if !s.Reqaddr1.IsUnset() {
			vals[45] = psql.Arg(s.Reqaddr1.MustGetNull())
		} else {
			vals[45] = psql.Raw("DEFAULT")
		}

		if !s.Reqaddr2.IsUnset() {
			vals[46] = psql.Arg(s.Reqaddr2.MustGetNull())
		} else {
			vals[46] = psql.Raw("DEFAULT")
		}

		if !s.Reqcity.IsUnset() {
			vals[47] = psql.Arg(s.Reqcity.MustGetNull())
		} else {
			vals[47] = psql.Raw("DEFAULT")
		}

		if !s.Reqcompany.IsUnset() {
			vals[48] = psql.Arg(s.Reqcompany.MustGetNull())
		} else {
			vals[48] = psql.Raw("DEFAULT")
		}

		if !s.Reqcrossst.IsUnset() {
			vals[49] = psql.Arg(s.Reqcrossst.MustGetNull())
		} else {
			vals[49] = psql.Raw("DEFAULT")
		}

		if !s.Reqdescr.IsUnset() {
			vals[50] = psql.Arg(s.Reqdescr.MustGetNull())
		} else {
			vals[50] = psql.Raw("DEFAULT")
		}

		if !s.Reqfldnotes.IsUnset() {
			vals[51] = psql.Arg(s.Reqfldnotes.MustGetNull())
		} else {
			vals[51] = psql.Raw("DEFAULT")
		}

		if !s.Reqmapgrid.IsUnset() {
			vals[52] = psql.Arg(s.Reqmapgrid.MustGetNull())
		} else {
			vals[52] = psql.Raw("DEFAULT")
		}

		if !s.Reqnotesforcust.IsUnset() {
			vals[53] = psql.Arg(s.Reqnotesforcust.MustGetNull())
		} else {
			vals[53] = psql.Raw("DEFAULT")
		}

		if !s.Reqnotesfortech.IsUnset() {
			vals[54] = psql.Arg(s.Reqnotesfortech.MustGetNull())
		} else {
			vals[54] = psql.Raw("DEFAULT")
		}

		if !s.Reqpermission.IsUnset() {
			vals[55] = psql.Arg(s.Reqpermission.MustGetNull())
		} else {
			vals[55] = psql.Raw("DEFAULT")
		}

		if !s.Reqprogramactions.IsUnset() {
			vals[56] = psql.Arg(s.Reqprogramactions.MustGetNull())
		} else {
			vals[56] = psql.Raw("DEFAULT")
		}

		if !s.Reqstate.IsUnset() {
			vals[57] = psql.Arg(s.Reqstate.MustGetNull())
		} else {
			vals[57] = psql.Raw("DEFAULT")
		}

		if !s.Reqsubdiv.IsUnset() {
			vals[58] = psql.Arg(s.Reqsubdiv.MustGetNull())
		} else {
			vals[58] = psql.Raw("DEFAULT")
		}

		if !s.Reqtarget.IsUnset() {
			vals[59] = psql.Arg(s.Reqtarget.MustGetNull())
		} else {
			vals[59] = psql.Raw("DEFAULT")
		}

		if !s.Reqzip.IsUnset() {
			vals[60] = psql.Arg(s.Reqzip.MustGetNull())
		} else {
			vals[60] = psql.Raw("DEFAULT")
		}

		if !s.Responsedaycount.IsUnset() {
			vals[61] = psql.Arg(s.Responsedaycount.MustGetNull())
		} else {
			vals[61] = psql.Raw("DEFAULT")
		}

		if !s.Reviewed.IsUnset() {
			vals[62] = psql.Arg(s.Reviewed.MustGetNull())
		} else {
			vals[62] = psql.Raw("DEFAULT")
		}

		if !s.Reviewedby.IsUnset() {
			vals[63] = psql.Arg(s.Reviewedby.MustGetNull())
		} else {
			vals[63] = psql.Raw("DEFAULT")
		}

		if !s.Revieweddate.IsUnset() {
			vals[64] = psql.Arg(s.Revieweddate.MustGetNull())
		} else {
			vals[64] = psql.Raw("DEFAULT")
		}

		if !s.Scheduled.IsUnset() {
			vals[65] = psql.Arg(s.Scheduled.MustGetNull())
		} else {
			vals[65] = psql.Raw("DEFAULT")
		}

		if !s.Scheduleddate.IsUnset() {
			vals[66] = psql.Arg(s.Scheduleddate.MustGetNull())
		} else {
			vals[66] = psql.Raw("DEFAULT")
		}

		if !s.Source.IsUnset() {
			vals[67] = psql.Arg(s.Source.MustGetNull())
		} else {
			vals[67] = psql.Raw("DEFAULT")
		}

		if !s.SRNumber.IsUnset() {
			vals[68] = psql.Arg(s.SRNumber.MustGetNull())
		} else {
			vals[68] = psql.Raw("DEFAULT")
		}

		if !s.Status.IsUnset() {
			vals[69] = psql.Arg(s.Status.MustGetNull())
		} else {
			vals[69] = psql.Raw("DEFAULT")
		}

		if !s.Supervisor.IsUnset() {
			vals[70] = psql.Arg(s.Supervisor.MustGetNull())
		} else {
			vals[70] = psql.Raw("DEFAULT")
		}

		if !s.Techclosed.IsUnset() {
			vals[71] = psql.Arg(s.Techclosed.MustGetNull())
		} else {
			vals[71] = psql.Raw("DEFAULT")
		}

		if !s.Validx.IsUnset() {
			vals[72] = psql.Arg(s.Validx.MustGetNull())
		} else {
			vals[72] = psql.Raw("DEFAULT")
		}

		if !s.Validy.IsUnset() {
			vals[73] = psql.Arg(s.Validy.MustGetNull())
		} else {
			vals[73] = psql.Raw("DEFAULT")
		}

		if !s.Xvalue.IsUnset() {
			vals[74] = psql.Arg(s.Xvalue.MustGetNull())
		} else {
			vals[74] = psql.Raw("DEFAULT")
		}

		if !s.Yvalue.IsUnset() {
			vals[75] = psql.Arg(s.Yvalue.MustGetNull())
		} else {
			vals[75] = psql.Raw("DEFAULT")
		}

		if !s.Zone.IsUnset() {
			vals[76] = psql.Arg(s.Zone.MustGetNull())
		} else {
			vals[76] = psql.Raw("DEFAULT")
		}

		if !s.Zone2.IsUnset() {
			vals[77] = psql.Arg(s.Zone2.MustGetNull())
		} else {
			vals[77] = psql.Raw("DEFAULT")
		}

		if !s.CreatedDate.IsUnset() {
			vals[78] = psql.Arg(s.CreatedDate.MustGetNull())
		} else {
			vals[78] = psql.Raw("DEFAULT")
		}

		if !s.CreatedUser.IsUnset() {
			vals[79] = psql.Arg(s.CreatedUser.MustGetNull())
		} else {
			vals[79] = psql.Raw("DEFAULT")
		}

		if !s.GeometryX.IsUnset() {
			vals[80] = psql.Arg(s.GeometryX.MustGetNull())
		} else {
			vals[80] = psql.Raw("DEFAULT")
		}

		if !s.GeometryY.IsUnset() {
			vals[81] = psql.Arg(s.GeometryY.MustGetNull())
		} else {
			vals[81] = psql.Raw("DEFAULT")
		}

		if !s.LastEditedDate.IsUnset() {
			vals[82] = psql.Arg(s.LastEditedDate.MustGetNull())
		} else {
			vals[82] = psql.Raw("DEFAULT")
		}

		if !s.LastEditedUser.IsUnset() {
			vals[83] = psql.Arg(s.LastEditedUser.MustGetNull())
		} else {
			vals[83] = psql.Raw("DEFAULT")
		}

		if !s.Dog.IsUnset() {
			vals[84] = psql.Arg(s.Dog.MustGetNull())
		} else {
			vals[84] = psql.Raw("DEFAULT")
		}

		if !s.Spanish.IsUnset() {
			vals[85] = psql.Arg(s.Spanish.MustGetNull())
		} else {
			vals[85] = psql.Raw("DEFAULT")
		}

		if !s.ScheduleNotes.IsUnset() {
			vals[86] = psql.Arg(s.ScheduleNotes.MustGetNull())
		} else {
			vals[86] = psql.Raw("DEFAULT")
		}

		if !s.SchedulePeriod.IsUnset() {
			vals[87] = psql.Arg(s.SchedulePeriod.MustGetNull())
		} else {
			vals[87] = psql.Raw("DEFAULT")
		}

		if s.Updated.IsValue() {
			vals[88] = psql.Arg(s.Updated.MustGet())
		} else {
...
func (s FSServicerequestSetter) Expressions(prefix ...string) []bob.Expression {
	exprs := make([]bob.Expression, 0, 89)
