...
func (s *HistoryLinelocationSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if !s.LengthFT.IsUnset() {
			vals[29] = psql.Arg(s.LengthFT.MustGetNull())
		} else {
			vals[29] = psql.Raw("DEFAULT")
		}

		if !s.LengthMeters.IsUnset() {
			vals[30] = psql.Arg(s.LengthMeters.MustGetNull())
		} else {
		...
		if !s.WidthFT.IsUnset() {
			vals[40] = psql.Arg(s.WidthFT.MustGetNull())
		} else {
			vals[40] = psql.Raw("DEFAULT")
		}

		if !s.WidthMeters.IsUnset() {
			vals[41] = psql.Arg(s.WidthMeters.MustGetNull())
		} else {
			vals[41] = psql.Raw("DEFAULT")
		}

		if !s.Zone.IsUnset() {
			vals[42] = psql.Arg(s.Zone.MustGetNull())
		} else {
			vals[42] = psql.Raw("DEFAULT")
		}

		if !s.Zone2.IsUnset() {
			vals[43] = psql.Arg(s.Zone2.MustGetNull())
		} else {
			vals[43] = psql.Raw("DEFAULT")
		}

		if !s.Created.IsUnset() {
			vals[44] = psql.Arg(s.Created.MustGetNull())
		} else {
