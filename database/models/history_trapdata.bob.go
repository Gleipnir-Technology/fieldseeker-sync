...
func buildHistoryTrapdatumColumns(alias string) historyTrapdatumColumns {
	return historyTrapdatumColumns{
		ColumnsExpr: expr.NewColumnsExpr(
			"avetemp", "comments", "creationdate", "creator", "enddatetime", "editdate", "editor", "fieldtech", "field", "gatewaysync", "globalid", "idbytech", "locationname", "loc_id", "lr", "lure", "objectid", "processed", "raingauge", "recordstatus", "reviewed", "reviewedby", "revieweddate", "sitecond", "sortbytech", "srid", "startdatetime", "trapactivitytype", "trapcondition", "trapnights", "traptype", "voltage", "vectorsurvtrapdataid", "vectorsurvtraplocationid", "winddir", "windspeed", "zone", "zone2", "created", "created_date", "created_user", "geometry_x", "geometry_y", "last_edited_date", "last_edited_user", "version",
...
func (s *HistoryTrapdatumSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if !s.Lure.IsUnset() {
			vals[15] = psql.Arg(s.Lure.MustGetNull())
		} else {
		...
		if !s.Vectorsurvtrapdataid.IsUnset() {
			vals[32] = psql.Arg(s.Vectorsurvtrapdataid.MustGetNull())
		} else {
			vals[32] = psql.Raw("DEFAULT")
		}

		if !s.Vectorsurvtraplocationid.IsUnset() {
			vals[33] = psql.Arg(s.Vectorsurvtraplocationid.MustGetNull())
		} else {
			vals[33] = psql.Raw("DEFAULT")
		}

		if !s.Winddir.IsUnset() {
			vals[34] = psql.Arg(s.Winddir.MustGetNull())
		} else {
			vals[34] = psql.Raw("DEFAULT")
		}

		if !s.Windspeed.IsUnset() {
			vals[35] = psql.Arg(s.Windspeed.MustGetNull())
		} else {
			vals[35] = psql.Raw("DEFAULT")
		}

		if !s.Zone.IsUnset() {
			vals[36] = psql.Arg(s.Zone.MustGetNull())
		} else {
			vals[36] = psql.Raw("DEFAULT")
		}

		if !s.Zone2.IsUnset() {
			vals[37] = psql.Arg(s.Zone2.MustGetNull())
		} else {
			vals[37] = psql.Raw("DEFAULT")
		}

		if !s.Created.IsUnset() {
			vals[38] = psql.Arg(s.Created.MustGetNull())
		} else {
			vals[38] = psql.Raw("DEFAULT")
		}

		if !s.CreatedDate.IsUnset() {
			vals[39] = psql.Arg(s.CreatedDate.MustGetNull())
		} else {
			vals[39] = psql.Raw("DEFAULT")
		}

		if !s.CreatedUser.IsUnset() {
			vals[40] = psql.Arg(s.CreatedUser.MustGetNull())
		} else {
		...
		if !s.LastEditedDate.IsUnset() {
			vals[43] = psql.Arg(s.LastEditedDate.MustGetNull())
		} else {
			vals[43] = psql.Raw("DEFAULT")
		}

		if !s.LastEditedUser.IsUnset() {
			vals[44] = psql.Arg(s.LastEditedUser.MustGetNull())
		} else {
