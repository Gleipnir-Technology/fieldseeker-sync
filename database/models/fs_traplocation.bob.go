...
func (s *FSTraplocationSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if !s.Route.IsUnset() {
			vals[26] = psql.Arg(s.Route.MustGetNull())
		} else {
			vals[26] = psql.Raw("DEFAULT")
		}

		if !s.RouteOrder.IsUnset() {
			vals[27] = psql.Arg(s.RouteOrder.MustGetNull())
		} else {
			vals[27] = psql.Raw("DEFAULT")
		}

		if !s.SetDow.IsUnset() {
			vals[28] = psql.Arg(s.SetDow.MustGetNull())
		} else {
			vals[28] = psql.Raw("DEFAULT")
		}

		if !s.Vectorsurvsiteid.IsUnset() {
			vals[29] = psql.Arg(s.Vectorsurvsiteid.MustGetNull())
		} else {
			vals[29] = psql.Raw("DEFAULT")
		}

		if !s.H3R7.IsUnset() {
			vals[30] = psql.Arg(s.H3R7.MustGetNull())
		} else {
			vals[30] = psql.Raw("DEFAULT")
		}

		if !s.H3R8.IsUnset() {
			vals[31] = psql.Arg(s.H3R8.MustGetNull())
		} else {
