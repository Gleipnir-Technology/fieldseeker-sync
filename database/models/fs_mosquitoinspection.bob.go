...
func (s *FSMosquitoinspectionSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if !s.CreatedDate.IsUnset() {
			vals[51] = psql.Arg(s.CreatedDate.MustGetNull())
		} else {
			vals[51] = psql.Raw("DEFAULT")
		}

		if !s.CreatedUser.IsUnset() {
			vals[52] = psql.Arg(s.CreatedUser.MustGetNull())
		} else {
		...
		if !s.LastEditedDate.IsUnset() {
			vals[55] = psql.Arg(s.LastEditedDate.MustGetNull())
		} else {
			vals[55] = psql.Raw("DEFAULT")
		}

		if !s.LastEditedUser.IsUnset() {
			vals[56] = psql.Arg(s.LastEditedUser.MustGetNull())
		} else {
			vals[56] = psql.Raw("DEFAULT")
		}

		if !s.Adminaction.IsUnset() {
			vals[57] = psql.Arg(s.Adminaction.MustGetNull())
		} else {
			vals[57] = psql.Raw("DEFAULT")
		}

		if s.Updated.IsValue() {
			vals[58] = psql.Arg(s.Updated.MustGet())
		} else {
