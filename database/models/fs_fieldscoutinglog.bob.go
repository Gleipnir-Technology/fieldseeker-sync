...
func buildFSFieldscoutinglogColumns(alias string) fsFieldscoutinglogColumns {
	return fsFieldscoutinglogColumns{
		ColumnsExpr: expr.NewColumnsExpr(
			"creationdate", "creator", "editdate", "editor", "globalid", "objectid", "status", "created_date", "created_user", "geometry_x", "geometry_y", "last_edited_date", "last_edited_user", "updated",
...
func (s *FSFieldscoutinglogSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(bob.ExpressionFunc(func(ctx context.Context, w io.Writer, d bob.Dialect, start int) ([]any, error) {
		...
		if !s.CreatedUser.IsUnset() {
			vals[8] = psql.Arg(s.CreatedUser.MustGetNull())
		} else {
			vals[8] = psql.Raw("DEFAULT")
		}

		if !s.GeometryX.IsUnset() {
			vals[9] = psql.Arg(s.GeometryX.MustGetNull())
		} else {
			vals[9] = psql.Raw("DEFAULT")
		}

		if !s.GeometryY.IsUnset() {
			vals[10] = psql.Arg(s.GeometryY.MustGetNull())
		} else {
			vals[10] = psql.Raw("DEFAULT")
		}

		if !s.LastEditedDate.IsUnset() {
			vals[11] = psql.Arg(s.LastEditedDate.MustGetNull())
		} else {
			vals[11] = psql.Raw("DEFAULT")
		}

		if !s.LastEditedUser.IsUnset() {
			vals[12] = psql.Arg(s.LastEditedUser.MustGetNull())
		} else {
			vals[12] = psql.Raw("DEFAULT")
		}

		if s.Updated.IsValue() {
			vals[13] = psql.Arg(s.Updated.MustGet())
		} else {
