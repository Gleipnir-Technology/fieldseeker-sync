...
def main() -> None:
	...
	parser.add_argument("--dest", "-d", type=Path, default="tools/truncate-tables.sql", help="The output file to write to")
	parser.add_argument("--src", "-s", type=Path, default="schema", help="The directory to pull schema from")
	args = parser.parse_args()

	logging.basicConfig(level=logging.DEBUG)

	# Parse all the schemas
	logging.info("Reading schema from %s", args.src)
	...
	logging.info("Writing truncate script to %s", args.dest)
	with open(args.dest, "w") as output:
		# Write out what we parsed
		for table_name in sorted(schema):
			output.write(f"TRUNCATE TABLE FS_{table_name};\n")
